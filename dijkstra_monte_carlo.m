% reset variables and command window
clear;
clc;
...
% -------------------------------------------------------------------------
CNT_num = input('Enter the number of carbon nanotubes:');
film_length = input('Enter the length dimension of the square film in um (Note: L = W):');
CNT_length = input('Enter mean nanotube length in um (L>>diameter):'); % mean of CNT length
CNT_lengthset = CNT_length + (CNT_length * 0.15) * randn(CNT_num,1); % length + variant term
le = input('Enter critical resistance length in um (e.g. 0.2 um):'); % Critical length for ballistic resistance regime 
Rj = input('Enter tube-tube junction resistance (kOhms):'); % Tube-tube junction resistance factor 
